...
This investigation explores the feasibility of migrating the Clautana extension from the current custom implementation (based on `@anthropic-ai/claude-agent-sdk`) to the **Microsoft Agent Framework** orchestration patterns.
...
| Concept | Current Implementation (Clautana) | Microsoft Agent Framework (Semantic Kernel) |
| :--- | :--- | :--- |
...
### 4.2. MCP Integration
Clautana relies on MCP. Semantic Kernel has experimental support for importing OpenAI plugins, but direct MCP support might require a custom adapter.
